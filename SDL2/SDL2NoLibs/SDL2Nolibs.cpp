...
SDL_bool CSDL::SetHint(const char* name, const char* value){
	return Instance()->SDL_SetHint(name, value);
}
SDL_bool CSDL::GetWindowWMInfo(SDL_Window* window, SDL_SysWMinfo* info) {
	return Instance()->SDL_GetWindowWMInfo(window, info);
}
uint32_t CSDL::GetWindowID(SDL_Window* window) {
	return Instance()->SDL_GetWindowID(window);
}
#pragma endregion
